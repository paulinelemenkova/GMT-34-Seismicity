...
#gmt makecpt -CGPS-Simple-Smoke.cpt -V -T-164/680 > pauline.cpt
#gmt makecpt -Csaga-03.cpt -V -T-164/680 > pauline.cpt
#gmt makecpt -Cinferno.cpt -V -T-164/680 > pauline.cpt
#gmt makecpt -Cblue-green-metal.cpt -V -T-164/680 > pauline.cpt
#gmt makecpt -Cwarm_gray_dk.cpt -V -T-164/680 > pauline.cpt
#gmt makecpt -Csilver_01.cpt -V -T-164/680 > pauline.cpt
#gmt makecpt -Cgrayscale_01.cpt -V -T-164/680 > pauline.cpt
#gmt makecpt -CGreys_09.cpt -V -T-164/680 > pauline.cpt
#gmt makecpt -Cgray -V -T-164/680 > pauline.cpt
gmt makecpt -Cpm3d05.cpt -V -T-164/680 > pauline.cpt
gmt makecpt -Cseis -T1.0/5.5/0.5 -Z > steps.cpt
# srtm dem1, dem2, dem3

ps=Seis_BE.ps
# Make raster image
gmt grdimage be_relief.nc -Cpauline.cpt -R2.5/6.5/49.5/51.7 -JB4.30/50.30/50/55/6.5i -I+a15+ne0.75 -Xc -P -K > $ps
...
gmt psbasemap -R -J \
    ...
    --MAP_ANNOT_OFFSET=0.1c \
    -Lx14.0c/-3.7c+c10+w75k+l"Albers conic equal-area projection. Scale (km)"+f \
    -UBL/0p/-110p -O -K >> $ps
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f15p,0,khaki1+jLB >> $ps << EOF
2.87 50.05 F  R  A  N  C  E
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f13p,0,khaki1+jLB >> $ps << EOF
4.45 51.51 N E T H E R L A N D S
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f12p,0,khaki1+jLB+a-270 >> $ps << EOF
6.38 50.6 G E R M A N Y
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f9p,0,khaki1+jLB >> $ps << EOF
5.85 49.70 LUXEMBOURG
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f11p,2,azure1+jLB >> $ps << EOF
2.6 51.5 NORTH SEA
EOF

gmt pslegend -R -J -Dx2.0/-3.1+w17.0c+o-2.0/0.1c \
...
# Add subtitle
gmt pstext -R0/10/0/15 -JX10/10 -X0.0c -Y8.8c -N -O \
    -F+f11p,25,black+jLB >> $ps << EOF
0.8 9.0 DEM: SRTM/GEBCO, 15 arc sec grid.
EOF
